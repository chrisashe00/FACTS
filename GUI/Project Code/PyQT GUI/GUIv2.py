...
class MyWindow(QMainWindow):
        ...
        if pixmap is not None:
            ...
            for y in range(img.height()):
                for x in range(img.width()):
                    ...
                    g = np.clip(g + value, 0, 255)
                    b = np.clip(b + value, 0, 255)
    ...
    def apply_contrast_to_pixmap(self, pixmap, value):
        if pixmap is None:
            return None  # Add this line to handle the case when pixmap is None
    ...
    def updateCamera(self):
        while self.running:
            try:
                ...
                QThread.msleep(30)  # Add this line for a small sleep duration
            except RuntimeError:
